...
from tensorflow.keras.layers import Input, Dense, Dropout, GlobalAveragePooling2D, Lambda, GaussianNoise, BatchNormalization, GlobalMaxPooling2D
...
def build_model(hp):
    ...
    model.add(tf.keras.layers.RandomBrightness(hp.Float('brightness', min_value=0.05, max_value=0.3, step=0.05)))
    model.add(tf.keras.layers.RandomContrast(hp.Float('contrast', min_value=0.05, max_value=0.3, step=0.05)))
...
def save_summary(tuner, phase):
    ...
    results += "\nAll trials:\n"
    for trial in tuner.oracle.get_best_trials(num_trials=12):
...
def run_hyperparameter_tuning(train_ds, val_ds):
    ...
    tuner_fine.search(train_ds, epochs=20, validation_data=val_ds, class_weight=class_weights, callbacks=[early_stopping_fine])
    ...
    history_fine = final_model.fit(train_ds, epochs=20, validation_data=val_ds, class_weight=class_weights, callbacks=[early_stopping_fine])
